...
var layers_nj = {
  ...
  // Create the map with our layers_nj
  var map_nj = L.map("map-nj", {
    center: [40.0583, -74.4059],
    zoom: 8,
    layers: [
      layers_nj.Arson,
      layers_nj.Campfire,
      layers_nj.Children,
      layers_nj.Debris_Burning,
      layers_nj.Equipment_Use,
      layers_nj.Fireworks,
      layers_nj.Lightning,
      layers_nj.Miscellaneous,
      layers_nj.Missing_Undefined,
      layers_nj.Powerline,
      layers_nj.Railroad,
      layers_nj.Smoking,
      layers_nj.Structure,
  ...
  lightmap.addTo(map_nj);
  
  // Create an overlays object to add to the layer control
  var overlays_nj = {
    "Arson": layers_nj.Arson,
    "Campfire": layers_nj.Campfire,
    "Children": layers_nj.Children,
    "Debris Burning": layers_nj.Debris_Burning,
    "Equipment Use": layers_nj.Equipment_Use,
    "Fireworks": layers_nj.Fireworks,
    "Lightning": layers_nj.Lightning,
    "Miscellaneous": layers_nj.Miscellaneous,
    "Missing/Undefined": layers_nj.Missing_Undefined,
    "Powerline": layers_nj.Powerline,
    "Railroad": layers_nj.Railroad,
    "Smoking": layers_nj.Smoking,
    "Structure": layers_nj.Structure,
    "Other": layers_nj.Other
  };

  // Create a control for our layers_nj, add our overlay layers_nj to it
  L.control.layers(null, overlays_nj).addTo(map_nj);
  ...
  info.onAdd = function() {
    var div = L.DomUtil.create("div", "legend_nj");
    return div;
  };
  ...
  d3.json("http://127.0.0.1:5000/api/fire_log_nj", function(fireLog) {
      ...
      // Initialize a stationStatusCode, which will be used as a key to access the appropriate layers_nj, icons, and station count for layer group
      var firetypeCode;
      ...
      for (var i = 0; i < fireLog.length; i++) {
        ...
        newMarker.addTo(layers_nj[firetypeCode]);
      ...
      updateLegend_nj(fireCount);
    });

  
  // Update the legend's innerHTML with the last updated time and station count
  function updateLegend_nj(fireCount) {
    document.querySelector(".legend_nj").innerHTML = [
