...
var layers_ri = {
  ...
  // Create the map with our layers_ri
  var map_ri = L.map("map-ri", {
    center: [41.58, -71.4774],
    zoom: 9,
    layers: [
      layers_ri.Arson,
      layers_ri.Campfire,
      layers_ri.Children,
      layers_ri.Debris_Burning,
      layers_ri.Equipment_Use,
      layers_ri.Fireworks,
      layers_ri.Lightning,
      layers_ri.Miscellaneous,
      layers_ri.Missing_Undefined,
      layers_ri.Powerline,
      layers_ri.Railroad,
      layers_ri.Smoking,
      layers_ri.Structure,
  ...
  lightmap.addTo(map_ri);
  
  // Create an overlays object to add to the layer control
  var overlays_ri = {
    "Arson": layers_ri.Arson,
    "Campfire": layers_ri.Campfire,
    "Children": layers_ri.Children,
    "Debris Burning": layers_ri.Debris_Burning,
    "Equipment Use": layers_ri.Equipment_Use,
    "Fireworks": layers_ri.Fireworks,
    "Lightning": layers_ri.Lightning,
    "Miscellaneous": layers_ri.Miscellaneous,
    "Missing/Undefined": layers_ri.Missing_Undefined,
    "Powerline": layers_ri.Powerline,
    "Railroad": layers_ri.Railroad,
    "Smoking": layers_ri.Smoking,
    "Structure": layers_ri.Structure,
    "Other": layers_ri.Other
  };

  // Create a control for our layers_ri, add our overlay layers_ri to it
  L.control.layers(null, overlays_ri).addTo(map_ri);
  ...
  info.onAdd = function() {
    var div = L.DomUtil.create("div", "legend_ri");
    return div;
  };
  ...
  d3.json("http://127.0.0.1:5000/api/fire_log_ri", function(fireLog) {
      ...
      // Initialize a stationStatusCode, which will be used as a key to access the appropriate layers_ri, icons, and station count for layer group
      var firetypeCode;
      ...
      for (var i = 0; i < fireLog.length; i++) {
        ...
        newMarker.addTo(layers_ri[firetypeCode]);
      ...
      updateLegend_ri(fireCount);
    });

  
  // Update the legend's innerHTML with the last updated time and station count
  function updateLegend_ri(fireCount) {
    document.querySelector(".legend_ri").innerHTML = [
